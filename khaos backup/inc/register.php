...
if (isset($_POST['method']) && $_POST['method'] == 'discord') {
    ...
    if ($stmt = $dbcon->prepare('SELECT * FROM users where users_username = ?')) {
        ...
        if ($stmt->num_rows > 0) {
            ...
            header("Location: " . $_CONFIG['domain'] . "/register.php?status=error&msg=" . $msg);
        } else {
            ...
            if ($result) {
                ...
                if ($result2) {
                    ...
                    header("location: " . $_CONFIG['domain'] . "/register.php?status=error");
                }
            } else {
                header("location: " . $_CONFIG['domain'] . "/register.php?status=error");
            }
            ...
            if ($result) {
                ...
                if ($result2) {
                    ...
                    $message = "
                        ...
                        <p><a href='" . $_CONFIG['domain'] . "/verification.php?email=" . $email . "&code=" . $verificationcode . "' target='_blank'>" . $_CONFIG['domain'] . "/verification.php?email=" . $email . "&code=" . $verificationcode . "</a></p>
                        <br>
